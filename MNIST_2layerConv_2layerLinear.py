...
2. define layers and activation functions.
   You should define each Convolution, Linear, max_pooling layer but just define activation functoin once.
...
for epoch in range(100):
    ...
    for i in range(len(valid_set[1])):
        img = np.reshape(valid_set[0][index], (-1, 28, 28))  # input data reshaping
        output = model.forward(img)
...
for i in range(len(test_set[1])):
    img = np.reshape(test_set[0][index], (-1, 28, 28))  # input data reshaping
    output = model.forward(img)
    nn.dcg.clear()
